...
import {IAccessControlManagerV5} from "../../Governance/IGoverance.sol";
    ...
    Exponential
{
    ...
    event MintOnlyForPrimeHolder(
        bool previousMintEnabledOnlyForPrimeHolder,
        bool newMintEnabledOnlyForPrimeHolder
    );
    ...
    event NewVAIFloatRate(
        uint256 oldFloatRateMantissa,
        uint256 newFlatRateMantissa
    );
    ...
    event NewAccessControl(
        address oldAccessControlAddress,
    ...
    function mintVAI(
        ...
        require(
            ...
            "could not compute mintable amount"
        );

        // check that user have sufficient mintableVAI balance
        require(
            mintVAIAmount <= accountMintableVAI,
        ...
        if (treasuryPercent != 0) {
            uint256 feeAmount = div_(
                mul_(mintVAIAmount, treasuryPercent),
                1e18
            );
    ...
    function repayVAIFresh(
        ...
        (
            uint256 burn,
            uint256 partOfCurrentInterest,
            uint256 partOfPastInterest
        ) = getVAICalculateRepayAmount(borrower, repayAmount);

        VAI _vai = VAI(vai);
        _vai.burn(payer, burn);
        bool success = _vai.transferFrom(
            payer,
            receiver,
        ...
        uint256 accountVAINew = sub_(
            sub_(vaiBalanceBorrower, burn),
            partOfPastInterest
        );
        pastVAIInterest[borrower] = sub_(
    ...
    function liquidateVAIFresh(
        ...
        if (address(comptroller) != address(0)) {
            ...
            ) = repayVAIFresh(liquidator, borrower, repayAmount);
            ...
            require(
                seizeError == uint256(Error.NO_ERROR),
    ...
    function getVAIRepayRate() public view returns (uint256) {
        ...
        if (baseRateMantissa > 0) {
            if (floatRateMantissa > 0) {
                uint256 oraclePrice = oracle.getUnderlyingPrice(
                    VToken(getVAIAddress())
                );
                if (1e18 > oraclePrice) {
                    uint256 delta;
                    uint256 rate;
    ...
    function getVAIRepayAmount(address account) public view returns (uint256) {
        ...
        (mErr, delta) = subUInt(
            vaiMintIndex,
            getVAIMinterInterestIndex(account)
        );
        require(
            mErr == MathError.NO_ERROR,
            "VAI_TOTAL_REPAY_AMOUNT_CALCULATION_FAILED"
    ...
    function getVAICalculateRepayAmount(
        ...
        (mErr, currentInterest) = subUInt(
            totalRepayAmount,
            comptroller.mintedVAIs(borrower)
        );
        require(
            mErr == MathError.NO_ERROR,
    ...
    function _ensureAllowed(string memory functionSig) private view {
        require(
            IAccessControlManagerV5(accessControl).isAllowedToCall(
                msg.sender,
                functionSig
            ),
            "access denied"
